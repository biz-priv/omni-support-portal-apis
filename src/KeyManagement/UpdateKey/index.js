    ...
    if (!event.code) {
        try {
            ...
            if ((getItemResult.Items).length) {
                const [apiKeyResult, updateItemResult] = await Promise.all([Dynamo.getapikey(get(event, 'body.CustomerId'), USAGEPLAN), Dynamo.updateItems(TOKENVALIDATORTABLE, { 'CustomerID': get(event, 'body.CustomerId'), 'ApiKey': getItemResult.Items[0].ApiKey }, 'set CustomerStatus = :x', { ':x': 'Inactive' })])
